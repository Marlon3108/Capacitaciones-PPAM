...
export default function GestorNuevosParticipantes() {
  ...
  const handleImportarMasivo = async () => {
    ...
    try {
      const { data: existentes, error: errorFetch } = await supabase
        .from("participantes")
        .select(
          "id, nombres_apellidos, ciudad, congregacion, codigo_unico, estado, telefono, categoria, punto_fijo",
        );
      ...
      datosBrutos.forEach((fila) => {
        ...
        if (
          fila[indicePrioridad] !== undefined &&
          fila[indicePrioridad] !== null
        ) {
        ...
        let categoriaCalculada =
          tipoImportacion === "nuevos"
            ? "nuevo_orientacion"
            : "viejo_sin_punto";
        let puntoFijoCalculado = null;

        if (tipoImportacion === "antiguos") {
          const puntoExcel = fila[indicePunto]
        ...
        const pExistente = existentes.find(
          (p) =>
            p.nombres_apellidos.toLowerCase() === nombreExcel.toLowerCase(),
        );
  ...
  const handleGuardarManual = async (e) => {
    ...
    if (!manualNombre || !manualCongregacion || !manualTelefono) {
      setMensaje({
        tipo: "error",
        texto: "Por favor completa Nombre, Congregación y Teléfono.",
      });
      return;
    }
    ...
    if (manualTipo === "antiguos" && !manualPuntoFijo) {
      setMensaje({
        tipo: "error",
        texto: "Para un participante antiguo, debes escribir su Punto Fijo.",
      });
      return;
    }
    ...
    try {
      ...
      if (existente) {
        setMensaje({
          tipo: "error",
          texto: `El participante "${manualNombre}" ya existe en el sistema.`,
        });
        setCargando(false);
        return;
      }
      ...
      let categoriaCalculada =
        manualTipo === "nuevos" ? "nuevo_orientacion" : "viejo_sin_punto";
      let puntoFijoCalculado = null;
      ...
      setMensaje({
        tipo: "exito",
        texto: `¡Participante ${manualNombre} agregado correctamente!`,
      });
      setManualNombre("");
      ...
      setMensaje({
        tipo: "error",
        texto: "Hubo un error al guardar el participante: " + error.message,
      });
    } finally {
      setCargando(false);
    }
    ...
    return (
      <div className="bg-red-50 border border-red-200 p-8 rounded-2xl flex flex-col items-center justify-center text-center max-w-md mx-auto mt-10">
        ...
        <p className="text-red-600">
          ...
          acción es exclusiva para Administradores o Coordinadores.
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-6 max-w-4xl mx-auto pb-12">
      <div>
        ...
        <p className="text-gray-500 mt-1">
          Sube el archivo Excel o registra personas manualmente al sistema.
        </p>
      </div>
      ...
      {mensaje && (
        <div
          className={`p-4 rounded-xl flex items-center text-sm font-medium ${mensaje.tipo === "error" ? "bg-red-50 text-red-700 border border-red-100" : "bg-green-50 text-green-700 border border-green-100"}`}
        >
          {mensaje.tipo === "error" ? (
      ...
      {modoIngreso === "masivo" && (
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
          <h2 className="text-xl font-semibold text-gray-800 mb-4">
            Paso 1: Cargar Plantilla Masiva
          </h2>
                ...
                <p className="mb-2 text-sm text-gray-500">
                  {isDragging ? (
                    ...
                    <>
                      ...
                      o arrastra el archivo Excel/CSV
                    </>
                  )}
                </p>
              </div>
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        Hermanos que ya servían y requieren capacitación en
                        sitio.
                      </p>
                    </div>
                  </label>
