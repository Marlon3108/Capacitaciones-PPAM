...
export default function ImportadorSheets() {
  ...
  const [columnaCategoria, setColumnaCategoria] = useState("");
  ...
  const normalizarBool = (valor) => {
    const val = (valor ?? "").toString().toUpperCase().trim();
    return ["TRUE", "VERDADERO", "SI", "1"].includes(val);
  };

  const obtenerCategoria = (fila, indiceCategoria, tipo, indicePunto) => {
    const cat =
      indiceCategoria >= 0
        ? (fila[indiceCategoria] ?? "").toString().trim()
        : "";
    if (cat) return cat;
    if (tipo === "antiguos") {
      const puntoExcel =
        indicePunto >= 0 ? (fila[indicePunto] ?? "").toString().trim() : "";
      return puntoExcel ? "viejo_punto_fijo" : "viejo_sin_punto";
    }
  ...
  const handleImportar = async () => {
    if (
      ...
      setMensaje({
        tipo: "error",
        texto:
          "Por favor, selecciona todas las columnas base requeridas (Nombre, Ciudad, Congregación, Teléfono y Prioridad).",
      });
      return;
    }
    ...
    if (tipoImportacion === "antiguos" && !columnaPuntoFijo) {
      setMensaje({
        tipo: "error",
        texto:
          "Has indicado que son participantes Antiguos. Debes seleccionar la columna de Punto Fijo.",
      });
      return;
    }
    ...
    if (!columnaCategoria) {
      setMensaje({
        tipo: "error",
        texto: "Selecciona también la columna Categoria.",
      });
      return;
    }
    ...
    const indicePunto =
      tipoImportacion === "antiguos"
        ...
        : -1;
    const indiceCategoria = encabezados.indexOf(columnaCategoria);

    try {
      const { data: existentes, error: errorFetch } = await supabase
        .from("participantes")
        .select(
          "id, nombres_apellidos, ciudad, congregacion, codigo_unico, estado, telefono, categoria, punto_fijo, es_prioridad, orientacion_escaneado, orientacion_fecha_escaneo, invitacion_orientacion_enviada, qr_token",
        );
      ...
      datosBrutos.forEach((fila) => {
        const nombreExcel = fila[indiceNombre]
          ? fila[indiceNombre].toString().trim()
          : "";
        if (!nombreExcel) return;

        const ciudadExcel = fila[indiceCiudad]
          ? fila[indiceCiudad].toString().trim()
          : "No especificada";
        const congregacionExcel = fila[indiceCongregacion]
          ? fila[indiceCongregacion].toString().trim()
          : "No especificada";
        const telefonoExcel = fila[indiceTelefono]
          ? fila[indiceTelefono].toString().trim()
          : null;
        const prioridadExcel = normalizarBool(fila[indicePrioridad]);
        const categoriaCalculada = obtenerCategoria(
          fila,
          indiceCategoria,
          tipoImportacion,
          indicePunto,
        ...
        const puntoExcel =
          indicePunto >= 0
            ? fila[indicePunto]
              ? fila[indicePunto].toString().trim()
              : ""
            : "";
        ...
        if (pExistente) {
          const requiereActualizar =
            ...
            pExistente.es_prioridad !== prioridadExcel;
          ...
          if (requiereActualizar) {
            existentesAActualizar.push({
              ...
              qr_token: pExistente.qr_token,
              invitacion_orientacion_enviada:
                pExistente.invitacion_orientacion_enviada,
              orientacion_escaneado: pExistente.orientacion_escaneado,
              orientacion_fecha_escaneo: pExistente.orientacion_fecha_escaneo,
            });
          }
        } else {
          nuevosAInsertar.push({
            codigo_unico: generarCodigoUnico(),
            qr_token: generarCodigoUnico(),
            ...
            estado: "pendiente",
            invitacion_orientacion_enviada:
              categoriaCalculada === "nuevo_orientacion" ? false : null,
            orientacion_escaneado: false,
            orientacion_fecha_escaneo: null,
          });
        }
      });
      ...
      setMensaje({
        tipo: "error",
        texto: "Error de base de datos: " + error.message,
      });
    }
  ...
  if (verificandoRol) {
    return (
      <div className="h-full flex items-center justify-center">
        <Loader2 className="animate-spin mr-2 text-blue-600" /> Verificando
        permisos...
      </div>
    );
  }
    ...
    return (
      <div className="bg-red-50 border border-red-200 p-8 rounded-2xl flex flex-col items-center justify-center text-center max-w-md mx-auto mt-10">
        ...
        <p className="text-red-600">
          No tienes los permisos necesarios para importar bases de datos. Esta
          acción es exclusiva para Administradores y Escritorio.
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-6 max-w-4xl mx-auto pb-12">
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        <h2 className="text-xl font-semibold text-gray-800 mb-4">
          Paso 1: Cargar Participantes
        </h2>
        <p className="text-gray-500 mb-6 text-sm">
          El sistema es inteligente: Si el participante ya existe,{" "}
          <strong>no lo duplicará</strong>, solo actualizará su información si
          detecta cambios. Si es nuevo, le generará un código.
        </p>
        ...
        {archivo && (
          <div className="mt-4 p-4 bg-green-50 border border-green-100 rounded-lg flex items-center text-green-700">
            ...
            <div className="truncate">
              Archivo cargado:{" "}
              <span className="font-medium ml-1">{archivo}</span>
            </div>
            <span className="ml-auto text-sm font-medium flex-shrink-0">
              ({datosBrutos.length} filas)
            </span>
          </div>
        )}
      </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 animate-in fade-in slide-in-from-bottom-4">
          <div className="mb-8 bg-blue-50 p-5 rounded-xl border border-blue-200">
            <h3 className="font-bold text-blue-900 mb-3 text-lg">
              Paso 2: ¿Qué tipo de lista estás subiendo?
            </h3>
            <div className="flex flex-col sm:flex-row gap-4">
              <label
                className={`flex-1 flex items-start p-4 rounded-xl border cursor-pointer transition-all ${tipoImportacion === "nuevos" ? "bg-white border-blue-500 shadow-md ring-2 ring-blue-100" : "bg-blue-50/50 border-blue-200 hover:bg-white/60"}`}
              >
                <input
                  type="radio"
                  name="tipo"
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Participantes recientes listos para ir a la reunión de
                    orientación.
                  </p>
                </div>
              </label>
              ...
              <label
                className={`flex-1 flex items-start p-4 rounded-xl border cursor-pointer transition-all ${tipoImportacion === "antiguos" ? "bg-white border-blue-500 shadow-md ring-2 ring-blue-100" : "bg-blue-50/50 border-blue-200 hover:bg-white/60"}`}
              >
                <input
                  type="radio"
                  name="tipo"
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Hermanos que ya servían y requieren capacitación en sitio.
                  </p>
                </div>
              </label>
            </div>
          ...
          <div className="space-y-4 max-w-2xl mb-8">
            <div className="flex items-center justify-between p-3 border border-gray-200 rounded-lg">
              ...
              <div className="w-1/2">
                <select
                  value={columnaNombre}
                  onChange={(e) => setColumnaNombre(e.target.value)}
            ...
            <div className="flex items-center justify-between p-3 border border-gray-200 rounded-lg">
              <div className="w-1/3">
                <span className="font-medium text-gray-700 text-sm">
                  Ciudad asignada <span className="text-red-500">*</span>
                </span>
              </div>
              <div className="w-10 flex justify-center text-gray-400">
                <LinkIcon size={16} />
              ...
              <div className="w-1/2">
                <select
                  value={columnaCiudad}
                  onChange={(e) => setColumnaCiudad(e.target.value)}
                  className="w-full p-2 border border-gray-300 rounded-lg text-sm bg-white outline-none focus:ring-2 focus:ring-blue-500"
                >
                  <option value="">-- Selecciona --</option>
                  {encabezados.map((enc) => (
            ...
            <div className="flex items-center justify-between p-3 border border-gray-200 rounded-lg">
              <div className="w-1/3">
                <span className="font-medium text-gray-700 text-sm">
                  Congregación <span className="text-red-500">*</span>
                </span>
              </div>
              <div className="w-10 flex justify-center text-gray-400">
                <LinkIcon size={16} />
              ...
              <div className="w-1/2">
                <select
                  value={columnaCongregacion}
                  onChange={(e) => setColumnaCongregacion(e.target.value)}
                  className="w-full p-2 border border-gray-300 rounded-lg text-sm bg-white outline-none focus:ring-2 focus:ring-blue-500"
                >
                  <option value="">-- Selecciona --</option>
                  {encabezados.map((enc) => (
            ...
            <div className="flex items-center justify-between p-3 border border-blue-200 rounded-lg bg-blue-50/50">
              ...
              <div className="w-10 flex justify-center text-blue-400">
                <LinkIcon size={16} />
              </div>
              <div className="w-1/2">
                <select
            ...
            <div className="flex items-center justify-between p-3 border border-amber-200 rounded-lg bg-amber-50/50">
              <div className="w-1/3">
                <span className="font-medium text-amber-900 text-sm">
                  Prioritario (Urgente) <span className="text-red-500">*</span>
                </span>
              </div>
              <div className="w-10 flex justify-center text-amber-400">
                <LinkIcon size={16} />
              </div>
              <div className="w-1/2">
                <select
                  value={columnaPrioridad}
                  onChange={(e) => setColumnaPrioridad(e.target.value)}
                  className="w-full p-2 border border-amber-300 rounded-lg text-sm bg-white outline-none focus:ring-2 focus:ring-amber-500"
                >
                  <option value="">-- Selecciona --</option>
                  {encabezados.map((enc) => (
                    <option key={enc} value={enc}>
            ...
            {tipoImportacion === "antiguos" && (
              <div className="flex items-center justify-between p-3 border border-indigo-200 rounded-lg bg-indigo-50 animate-in fade-in">
                ...
                <div className="w-10 flex justify-center text-indigo-400">
                  <LinkIcon size={16} />
                </div>
                <div className="w-1/2">
                  <select
                    value={columnaPuntoFijo}
                    onChange={(e) => setColumnaPuntoFijo(e.target.value)}
                    className="w-full p-2 border border-indigo-300 rounded-lg text-sm bg-white outline-none focus:ring-2 focus:ring-indigo-500"
                  >
            ...
            <div className="flex items-center justify-between p-3 border border-green-200 rounded-lg bg-green-50/50">
              ...
              <div className="w-10 flex justify-center text-green-400">
                <LinkIcon size={16} />
              </div>
              <div className="w-1/2">
                <select
                  value={columnaCategoria}
                  onChange={(e) => setColumnaCategoria(e.target.value)}
                  className="w-full p-2 border border-green-300 rounded-lg text-sm bg-white outline-none focus:ring-2 focus:ring-green-500"
                >
                  <option value="">-- Selecciona --</option>
                  {encabezados.map((enc) => (
                    <option key={enc} value={enc}>
          ...
          {mensaje && (
            <div
              className={`p-4 mb-6 rounded-lg flex items-start text-sm border ${mensaje.tipo === "error" ? "bg-red-50 text-red-700 border-red-200" : "bg-green-50 text-green-700 border-green-200"}`}
            >
              {mensaje.tipo === "error" ? (
          ...
          <div className="flex justify-end pt-4 border-t border-gray-100">
            ...
            >
              ...
              {cargando
                ...
                : "Procesar e Importar Participantes"}
            </button>
          </div>
        </div>
      )}
