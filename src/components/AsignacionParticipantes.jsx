...
import { Calendar, MapPin, User, Search, Save, X, Loader2, CheckCircle2, AlertTriangle, Phone, Star, RefreshCcw } from 'lucide-react'
...
export default function AsignacionParticipantes() {
  ...
  const cargarDatos = useCallback(async (modoManual = false) => {
    try {
      ...
      const { data: partData, error: partError } = await supabase
        ...
        .select(`
          ...
          orientacion_escaneado,
          orientacion_fecha_escaneo,
  ...
  const handleGuardarAsignacion = async (e) => {
    e.preventDefault()
    if (!participanteSeleccionado?.id) return
    setGuardando(true)
  ...
  const marcarComoPrioridad = async (id, estadoActual) => {
    ...
    setParticipantes(prev => prev.map(p => p.id === id ? { ...p, es_prioridad: nuevoEstado } : p))
    ...
    if (error) {
      ...
      setParticipantes(prev => prev.map(p => p.id === id ? { ...p, es_prioridad: estadoActual } : p))
    }
  }

  const participantesAMostrar = useMemo(() => {
    return participantes.filter(p => {
      if (p.categoria === 'nuevo_orientacion' && p.orientacion_escaneado !== true) return false
      if (vistaActual === 'pendientes' && p.capacitador_id) return false
      if (vistaActual === 'asignados' && !p.capacitador_id) return false

      const textoBusqueda = busqueda.toLowerCase()
      const coincideTexto =
        (p.nombres_apellidos?.toLowerCase() || '').includes(textoBusqueda) ||
      ...
      if (filtroCategoria === 'nuevo') {
        if (!(p.categoria === 'nuevo' || p.categoria === 'nuevo_orientacion')) return false
      } else if (filtroCategoria !== 'todos' && p.categoria !== filtroCategoria) {
        return false
      }
  ...
  const renderBadgeCategoria = (categoria) => {
    ...
    <div className="space-y-6 pb-10">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">Asigna, visualiza o reasigna capacitadores y fechas. Los nuevos aparecen aquí solo después de asistir a orientación.</p>
        </div>
        ...
        <div className="flex bg-gray-200 p-1 rounded-xl">
          ...
          >
            Sin Programar ({participantes.filter(p => (p.categoria !== 'nuevo_orientacion' || p.orientacion_escaneado === true) && !p.capacitador_id).length})
          </button>
          <button
          ...
          >
            Ya Asignados ({participantes.filter(p => (p.categoria !== 'nuevo_orientacion' || p.orientacion_escaneado === true) && p.capacitador_id).length})
          </button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        <div className="flex flex-col xl:flex-row gap-4 mb-6 justify-between items-start xl:items-center">
          <div className="relative flex-1 w-full xl:max-w-md">
            ...
            <input
              type="text"
              placeholder={vistaActual === 'asignados' ? 'Buscar participante o capacitador...' : 'Buscar por nombre o punto...'}
              value={busqueda}
        ...
        {participantesAMostrar.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {participantesAMostrar.map(p => (
              <div
                key={p.id}
                className={`p-4 rounded-xl border relative flex flex-col justify-between transition-all hover:shadow-md ${p.es_prioridad ? 'border-amber-200 bg-amber-50/30' : (p.capacitador_id ? 'border-green-200 bg-green-50/50' : 'border-gray-200 bg-white')} shadow-sm`}
              >
                <button
                ...
                <div className="pr-8">
                  ...
                  <div className="flex flex-wrap gap-2 items-center mt-1">
                    {renderBadgeCategoria(p.categoria)}
                    {p.categoria === 'nuevo_orientacion' && p.orientacion_escaneado && (
                      <span className="text-[10px] font-bold bg-emerald-50 text-emerald-700 border border-emerald-200 px-2 py-1 rounded w-fit mt-1">
                        Asistió a orientación
                      </span>
                    )}
                  </div>
            ...
            <form onSubmit={handleGuardarAsignacion} className="p-6 space-y-4">
              <div className="bg-blue-50 p-3 rounded-lg border border-blue-100 mb-4">
                ...
                <div className="flex gap-2 mt-1 flex-wrap">
                  ...
                  {participanteSeleccionado.categoria === 'nuevo_orientacion' && <span className="inline-block bg-green-100 text-green-700 text-xs px-2 py-0.5 rounded font-medium">Asistió a orientación</span>}
                </div>
              </div>
